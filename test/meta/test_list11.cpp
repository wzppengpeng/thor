#include "thor/meta/list11/list11.h"

#include "thor/meta/mtest/mtest.h"

FIXTURE(TEST_TYPE_LIST11_AND_BASIC_FUNC) {
...
FIXTURE(TEST_TYPE_LIST11_NORMAL_FUNCTIONS) {
    SETUP() {
        using List = __type_list__(int, char, double, float);
...
FIXTURE(TEST_TYPE_LIST11_HIGH_ORDER_FUNCTIONS) {
    SETUP() {
        template<typename T>
    ...
    TEST("test map operation") {
        ...
        ASSERT_EQ(__tl_map__(EmptyList, ToSize), EmptyList);
    };
